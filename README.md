...
* Bypass YouTube "Sign in to confirm you're not a bot" restrictions using the BgUtils POT provider.
* Generate Jellyfin-compatible `.nfo` metadata for downloaded media.
* Add URLs through the optional Telegram bot flow.
...
```bash
docker run -d -p 8081:8081 -v /path/to/downloads:/downloads ghcr.io/tatoalo/metube_pot
```
...
* __SC_USE_FFMPEG__: Use ffmpeg instead of N_m3u8DL-RE for StreamingCommunity downloads. Defaults to `true`.
...
* __TELEGRAM_MAX_URLS_PER_MESSAGE__: Maximum URLs processed from a single Telegram message. Defaults to `10`.
