...
def get_format(download_type: str, codec: str, format: str, quality: str) -> str:
    ...
    if download_type == "video":
        ...
        vres = f"[height<={quality}]" if quality not in ("best", "best_remux", "worst") else ""
        vcombo = vres + vfmt
...
def get_opts(
    ...
    if download_type == "video" and format == "mp4" and quality == "best_remux":
        # Remove any custom format string so it cannot override get_format().
