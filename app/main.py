...
class Config:
    ...
    _BOOLEAN = ('DOWNLOAD_DIRS_INDEXABLE', 'CUSTOM_DIRS', 'CREATE_CUSTOM_DIRS', 'DELETE_FILE_ON_TRASHCAN', 'HTTPS', 'ENABLE_ACCESSLOG', 'ALLOW_YTDL_OPTIONS_OVERRIDES', 'SC_USE_FFMPEG', 'TELEGRAM_BOT_ENABLED')
...
def get_available_formats():
    """Legacy format list used by the Telegram bot configuration UI."""
    return [
        {
            "id": "any",
            "qualities": [
        ...
        {"id": "ios", "qualities": [{"id": "best"}]},
        {"id": "m4a", "qualities": [{"id": "best"}, {"id": "192"}, {"id": "128"}]},
        {"id": "mp3", "qualities": [{"id": "best"}, {"id": "320"}, {"id": "192"}, {"id": "128"}]},
        {"id": "opus", "qualities": [{"id": "best"}]},
        {"id": "wav", "qualities": [{"id": "best"}]},
        {"id": "flac", "qualities": [{"id": "best"}]},
        {"id": "thumbnail", "qualities": [{"id": "best"}]},
    ]
...
def parse_download_options(post: dict) -> dict:
    ...
    if download_type == 'video':
        ...
        allowed_video_qualities = {'best', 'worst', '2160', '1440', '1080', '720', '480', '360', '240'}
        if format == 'mp4':
            allowed_video_qualities.add('best_remux')
        if quality not in allowed_video_qualities:
            raise web.HTTPBadRequest(reason=f'quality must be one of {sorted(allowed_video_qualities)} for video')
