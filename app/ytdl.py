...
from dl_formats import AUDIO_FORMATS, get_format, get_opts
        ...
        while True:
            try:
                ...
                if retry_with_impersonate:
                    ...
                    log.info(f"Retrying with --impersonate flag for {self.info.title}")
                ...
                ) and not retry_with_impersonate:
                    log.warning(
                        f"Got 403 error for {self.info.title}, retrying with impersonate..."
                    )
                    retry_with_impersonate = True
                    time.sleep(10)  # Wait before retry
                    continue  # Retry

                # Final failure
                log.error(f"Download error for {self.info.title}: {error_msg}")
                self.status_queue.put({"status": "error", "msg": error_msg})
                break
...
class DownloadQueue:
    ...
    async def __add_entry(
        ...
        if etype.startswith("url"):
            ...
            if playlist_item_limit > 0:
                log.info(
                    f"Playlist item limit is set. Processing only first {playlist_item_limit} entries"
                )
    ...
    async def add(
        ...
        log.info(
            f"adding {url}: {quality=} {format=} {already=} {folder=} {custom_name_prefix=} {playlist_strict_mode=} {playlist_item_limit=} {auto_start=} {split_by_chapters=} {chapter_template=}"
        )
        ...
        try:
            entry = await asyncio.get_running_loop().run_in_executor(
                None, self.__extract_info, url, playlist_strict_mode
            )
        except yt_dlp.utils.YoutubeDLError as exc:
