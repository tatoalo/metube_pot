...
class DlFormatsTests(unittest.TestCase):
    ...
    def test_video_mp4_best_remux_uses_bestvideo_bestaudio(self):
        self.assertEqual(get_format("video", "auto", "mp4", "best_remux"), "bestvideo+bestaudio/best")
    ...
    def test_get_opts_best_remux_adds_audio_sync_postprocessor(self):
        opts = get_opts("video", "auto", "mp4", "best_remux", {"format": "custom"})
        keys = [p["key"] for p in opts["postprocessors"]]
        self.assertNotIn("format", opts)
        self.assertEqual(opts["merge_output_format"], "mp4")
        self.assertIn("FFmpegVideoConvertor", keys)
